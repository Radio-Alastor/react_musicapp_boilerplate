...
function App() {
  ...
  // Manage application's data (state hooks)
  const [data, setData] = useState([]);                               // constant for use to search for music based on a track, album or artist
  ...
  const [playlistName, setPlaylistName] = useState("New Playlist");   // constant stores the title for the playlist
  const [saveStatus, setSaveStatus] = useState(false);                // constant stores the save status of application
  ...
  function onSavePlaylist(){
    ...
    // reset inputs on the page itself
    setSearchTerm("");
    setSearchResults([]);
    setPlaylistTracks([]);
